...
{
    if (TIM_GetIntStatus(LPC_TIM2, TIM_MR0_INT)) // Inicio del periodo (20ms)
    {
        TIM_ClearIntPending(LPC_TIM2, TIM_MR0_INT);
        GPIO_SetValue(0, (1<<SERVO_PIN)); // Pin HIGH
    }

    if (TIM_GetIntStatus(LPC_TIM2, TIM_MR1_INT)) // Fin del pulso (1ms - 2ms)
    {
        TIM_ClearIntPending(LPC_TIM2, TIM_MR1_INT);
        GPIO_ClearValue(0, (1<<SERVO_PIN)); // Pin LOW
    }
}
