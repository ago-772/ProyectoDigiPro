...
void ConfADC(void)
{
    ...
    // CORRECCIÓN CRÍTICA: En multicanal/Burst, usar la Interrupción Global ADINTEN.
    // Esto asegura que la IRQ se dispare después de la conversión completa de todos los canales.
    //ADC_InitConfig(LPC_ADC , ADC_ADINTEN, ENABLE);
...
// Lee valor del canal 5  (ejey)
    ...
    // --- AÑADIR ESTO ---
    // Prepara un buffer de texto
    char buffer[64];

    // Formatea el string (como un printf)
    sprintf(buffer, "S1: %d us, S2: %d us\r\n", (int)pulso, (int)pulso2);

    // Envía el string por UART
    UART_SendString(LPC_UART0, buffer);
    // --- FIN DE LO AÑADIDO ---
}
...
void TIMER2_IRQHandler(void)
{
    ...
    if (TIM_GetIntStatus(LPC_TIM2, TIM_MR2_INT)) {
        ...
        GPIO_ClearValue(0, (1<<SERVO_PIN2));				//poner en BAJO el pin P0.10
    }
}

/**
 * @brief Configura el periférico UART0
 *
 * TXD0 en P0.2
 * RXD0 en P0.3
 * Baudrate: 9600
...
void ConfUART(void)
{
    ...
    PINSEL_CFG_Type PinCf;

    // P0.2 como TXD0 (Función 1)
    PinCf.Portnum = 0;
    PinCf.Pinnum = 2;
    PinCf.Funcnum = 1;
    PinCf.Pinmode = PINSEL_PINMODE_PULLUP;
    PinCf.OpenDrain = PINSEL_PINMODE_NORMAL;
    PINSEL_ConfigPin(&PinCf);

    // P0.3 como RXD0 (Función 1)
    PinCfg.Pinnum = 3;
    // El resto de la configuración es idéntica
    PINSEL_ConfigPin(&PinCf);

    // --- 2. Configuración del Periférico UART ---
    ...
    // --- 3. Configuración de la FIFO (Buffer) ---
    UART_FIFO_CFG_Type fifoCfg;
    UART_FIFOConfigStructInit(&fifoCfg); // Carga valores por defecto
    
    // Inicializa la FIFO
    UART_FIFOConfig(LPC_UART0, &fifoCfg);

    // --- 4. Habilita la transmisión ---
    UART_TxCmd(LPC_UART0, ENABLE);
}
/**
 * @brief Envía una cadena de texto por el UART especificado.
 * @param UARTx: Periférico UART (ej: LPC_UART0)
 * @param str: Cadena de texto a enviar
 */
void UART_SendString(LPC_UART_TypeDef *UARTx, const char *str)
{
    UART_Send(UARTx, (uint8_t*)str, strlen(str), BLOCKING);
}
...
int main(){
    ...
    UART_SendString(LPC_UART0, "¡Brazo Robotico Iniciado!\r\n");
