...
void configGPIO_UART(void) {
    ...
    psel_conf.Pinnum = 2;    // TXD0
    psel_conf.Funcnum = 1;
    PINSEL_ConfigPin(&psel_conf);

    psel_conf.Pinnum = 3;    // RXD0
    psel_conf.Funcnum = 1;
    PINSEL_ConfigPin(&psel_conf);
}
...
void ADC_IRQHandler(void)
{
    ...
    if (ADC_ChannelGetStatus(LPC_ADC, ADC_CH2, 1)) {
        ...
        pulso3 = mapServo(valorADC3);
        TIM_UpdateMatchValue(LPC_TIM2, 3, pulso3);
    }

    if (ADC_ChannelGetStatus(LPC_ADC, ADC_CH3, 1)) {
        valorADC4 = ADC_ChannelGetData(LPC_ADC, ADC_CH3);
        pulso4 = mapServo(valorADC4);
        TIM_UpdateMatchValue(LPC_TIM3, 1, pulso4);   // MR1 TIMER3
    }
}
...
void TIMER2_IRQHandler(void)
{
    if (TIM_GetIntStatus(LPC_TIM2, TIM_MR0_INT))
    {
        ...
        {
            char msg[160];
            sprintf(msg, "Pulsos: %u %u %u %u\r\n", pulso, pulso2, pulso3, pulso4);
            UART_Send((LPC_UART_TypeDef*)LPC_UART0, (uint8_t*)msg, strlen(msg), BLOCKING);
            counter = 0;
        }
